...
foreach ($file_recs as $this_filerec) {
   ...
   $analysis_db->querystring = "  select gini(array_accum(qout)) from ( ";
   $analysis_db->querystring .= "    select thisdate, \"Qout\" as qout from tmp_analysis_tbl1 ";
   $analysis_db->querystring .= "    where thisdate >= '$anal_start' and thisdate <= '$anal_end' ";
   $analysis_db->querystring .= " ) as foo ";
   print("$analysis_db->querystring \n");
   $analysis_db->performQuery();
   ...
   $analysis_db->querystring = "  select gini(array_accum(qout)) from ( ";
   $analysis_db->querystring .= "    select thisdate, \"Qout\" as qout from tmp_analysis_tbl2 ";
   $analysis_db->querystring .= "    where thisdate >= '$anal_start' and thisdate <= '$anal_end' ";
   $analysis_db->querystring .= " ) as foo ";
   print("$analysis_db->querystring \n");
   $analysis_db->performQuery();
   $g2_6hr = $analysis_db->getRecordValue(1,'gini');
   ...
   if ($num > 0) {
      ...
      $cbp_listobject->querystring .= " values ('$uid', 'baseline', $g1, $g1_6hr, now()) ";
   }
   print("$cbp_listobject->querystring \n");
   $cbp_listobject->performQuery();
   $cbp_listobject->querystring = "  select count(*) as numrecs from iha_karst_analysis ";
   ...
   $num = $cbp_listobject->getRecordValue(1,'numrecs');
   if ($num > 0) {
      $cbp_listobject->querystring = "  update iha_karst_analysis set gini_coeff = $g2, gini_ts6 = $g2_6hr, last_updated = now() ";
      $cbp_listobject->querystring .= " where unique_id = '$uid' ";
      $cbp_listobject->querystring .= "    and scenario = 'current' ";
   } else {
      $cbp_listobject->querystring = "  insert into iha_karst_analysis (unique_id, scenario, gini_coeff, gini_ts6, last_updated) ";
      $cbp_listobject->querystring .= " values ('$uid', 'current', $g2, $g2_6hr , now() ) ";
   }
   print("$cbp_listobject->querystring \n");
   $cbp_listobject->performQuery();
