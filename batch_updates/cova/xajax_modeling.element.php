...
include_once("./config.cova.local.php");
...
function showStatus($formValues, $mins = 60) {
   ...
   // old school
   /*
   $listobject->querystring = " select status_flag, status_mesg, last_updated, element_key from system_status where process_ownerid = $userid and last_updated >= now() - interval '$mins minutes' ";
   ...
   */
   $eid = $formValues['elementid'];
   $listobject->querystring = " select status_flag, status_mesg, last_updated, element_key from system_status where  element_key = $eid and last_updated >= now() - interval '$mins minutes' ";
