...
function getChildComponentType($listobject, $elementid, $childtype, $limit = -1) {
   $listobject->querystring = "  select a.elementid, a.elemname, a.objectclass from scen_model_element as a, map_model_linkages as b ";
...
function getRunFile($listobject, $elementid, $runid) {
   $listobject->querystring = "  select a.elementid, a.elemname, b.output_file, b.run_date, b.starttime, b.endtime from scen_model_element as a, scen_model_run_elements as b ";
...
function verifyRunStatus($listobject, $elementid, $qrunid = '', $qhost = '', $timeout = 300) {
...
function getModelStatus($formValues) {
   ...
   $innerHTML .= "<ul class=\"mDashBoard\"><li class=\"mDashBoard\">&nbsp;&nbsp;&nbsp;<b>Editing: </b><i>$childname</i>";
   if ($type == 3) {
      # can be a container, offer a "promote to active model link
      $onclick = "last_tab['model_element']='model_element_data0'; last_button['model_element']='model_element_0'; show_next('map_window_data0', 'map_window_0', 'map_window'); document.forms['elementbrowser'].elements.scenarioid.value=$scenarioid; document.forms['elementbrowser'].elements.actiontype.value='edit';  document.forms['elementbrowser'].elements.activecontainerid.value=$elementid; document.forms['elementbrowser'].elements.elementid.value=$elementid; xajax_showModelDesktopView(xajax.getFormValues('elementbrowser')); ";
...
function saveObjectSubComponents($listobject, $thisobject, $elid, $overwrite=0 ) {
   $innerHTML = '';
...
function getElementPropertyValue($listobject, $elementid, $properties) {
   $propvals = array();
...
function getSessionTableNames($thisobject, $elementid, $runid = -1, $data_element = '') {
   ...
   return array('tablename'=>$session_table, 'filename'=>$filename, 'innerHTML'=>$innerHTML, 'run_date' => $run_date);
}
...
function loadSessionTable($thisobject, $elementid, $runid = -1, $data_element = '') {
   ...
   $lobj = array('innerHTML'=>'', 'session_table'=>'');
   // make table name
   $sinfo = getSessionTableNames($thisobject, $elementid, $runid, $data_element);
   $session_table = $sinfo['tablename'];
   $filename = $sinfo['filename'];
...
function compactSerializeObject($thisobject) {
...
function updateObjectProps($projectid, $elementid, $prop_array) {
   global $listobject;
   $innerHTML = '';
   $debugHTML = '';
   ...
   if (is_object($thisobject)) {
      ...
      if (!$compres['error']) {
         ...
         if (in_array('name', array_keys($prop_array))) {
            $listobject->querystring .= ", elemname = '" . $prop_array['name'] . "'";
         }
         $listobject->querystring .= " where elementid = $elementid ";
         if ($debug) {
            $debugHTML .= "$listobject->querystring ;<br>";
...
function addElementResult($formValues) {
   ...
   if ( ($elementid > 0) and !$saveasnew) {
      ...
      $listobject->querystring = "  update scen_model_element set elemname = '$objectname', ";
...
function unSerializeSingleModelObject($elementid) {
...
function unSerializeModelObject($elementid, $input_props = array(), $model_listobj = '', $cache_level = -1, $cache_id = -1, $current_level = -1) {
   ...
   switch ($cache_type) {
      ...
      if ($current_level >= $cache_level) {
         ...
         $listobject->querystring = "  select output_file from scen_model_run_elements ";
         $listobject->querystring .= " where elementid = $elementid ";
         $listobject->querystring .= "    and runid = $cache_id ";
      } else {
         $cache_type = 'disabled';
      ...
      case 'date':
         ...
         $listobject->querystring .= " where elementid = $elementid ";
         $listobject->querystring .= "    and runid = $cache_id ";
         $listobject->querystring .= "    and run_date >= '$cache_level' ";
         $listobject->querystring .= "    and starttime <= '$model_startdate' ";
         $listobject->querystring .= "    and endtime >= '$model_enddate' ";
      break;
   ...
   if ($order == 0) {
      $cache_type = 'disabled';
      $returnArray['error'] .= "$elementid is a Zero'th order element - cacheing disabled.<br>";
   }
...
function cloneModelElement($scenarioid, $elementid, $activecontainerid = -1) {
   global $listobject, $projectid, $userid, $debug, $usergroupids, $adminsetuparray;
   ...
   if (!($perms && 4)) {
      ...
      $innerHTML .= "<b>Error: </b> You do not have read access to this element.<br>";
      $outarr['elementid'] = -1;
   } else {
      $listobject->querystring = "  insert into scen_model_element(scenarioid, modelid, elemname, ";
      $listobject->querystring .= "    file_based, elem_xml, elem_path, objectclass, the_geom, elemprops, ";
      ...
      $listobject->querystring .= " select $scenarioid, modelid, elemname || ' (copy)', file_based, ";
      $listobject->querystring .= "    elem_xml, elem_path, objectclass, the_geom, elemprops, ";
